...
class Goodlayers_Personnal extends WP_Widget {
	...
	function Goodlayers_Personnal() {
		parent::WP_Widget('personnal-widget', __('Personnel Widget (Goodlayers)','gdl_back_office'), 
			array('description' => __('A widget that show personnel information', 'gdl_back_office')));  
	}
	...
	function widget( $args, $instance ) {
		...
		// Widget Content
		wp_reset_query();	
		$custom_posts = get_posts( array('post_type'=>'personnel', 'showposts'=>$show_num, 'personnel-category'=>$personnal_cat) );
				...
				<div class="personnal-widget-item">
					...
					<div class="personnal-widget-context">
						<div class="personnal-widget-info">
							...
							<?php
								$personnal_position = get_post_meta( $custom_post->ID, 'personnel-option-position', true);
	...
	function form( $instance ) {
		...
		<p>
			...
			<?php 	
			$category_list = get_category_list( 'personnel-category' ); 
